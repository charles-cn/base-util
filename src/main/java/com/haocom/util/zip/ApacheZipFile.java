...
class ApacheZipFile {
     ...
     */
    private Map<ZipEntry, OffsetEntry> entries = new ConcurrentHashMap<ZipEntry, OffsetEntry>(HASH_SIZE);

    /**
     * Maps String to ZipEntrys, name -> actual entry.
     */
    private Map<String, ZipEntry> nameMap = new ConcurrentHashMap<String, ZipEntry>(HASH_SIZE);
     ...
     */
    public ApacheZipFile(File f) throws IOException {
        this(f, null);
    }
     ...
     *             if an error occurs while reading the file.
     */
    public ApacheZipFile(String name) throws IOException {
     ...
     *             if an error occurs while reading the file.
     */
    public ApacheZipFile(String name, String encoding) throws IOException {
     ...
     *             if an error occurs while reading the file.
     */
    public ApacheZipFile(File f, String encoding) throws IOException {
     ...
     */
    public static void closeQuietly(ApacheZipFile zipfile) {
        if (zipfile != null) {
            try {
                zipfile.close();
            }
    ...
    public InputStream getInputStream(ZipEntry ze) throws IOException, ZipException {
        ...
        System.out.println("off:" + offsetEntry.dataOffset);
        System.out.println("size:" + ze.getCompressedSize());
    ...
    private void resolveLocalFileHeaderData() throws IOException {
        ...
        for (Map.Entry<ZipEntry, OffsetEntry> entry : entries.entrySet()) {
             ...
             * extraFieldLen));
             */
            System.out.println("boffset:" + offset);
